...
export default function AudioPlayer({ 
  ...
  // Reset local state when season changes
  useEffect(() => {
    setIsPlaying(false);
    setProgress(0);
  }, [season.id]);
  ...
  return (
    <div className="glass-card p-6 md:p-8 flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left shadow-[0_20px_80px_-15px_rgba(45,45,45,0.05)] border-none h-full">
      ...
      <div className="flex-1 flex flex-col justify-center w-full">
        ...
        <audio
  ref={audioRef}
  key={currentTrack.audioUrl} // <-- Thêm đúng dòng này!
  ...
  Your browser does not support the audio element.
        ...
        <div className="flex items-center justify-between flex-wrap gap-6">
          <div className="flex items-center gap-4 md:gap-8">
            ...
            <button onClick={() => onTrackChange(Math.max(0, currentTrackIndex - 1))} className="text-[#2d2d2d]/50 hover:text-[#2d2d2d] transition-colors">
            ...
            <button onClick={() => onTrackChange(Math.min(tracks.length - 1, currentTrackIndex + 1))} className="text-[#2d2d2d]/50 hover:text-[#2d2d2d] transition-colors">
